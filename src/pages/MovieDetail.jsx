...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovieDetail(){
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=632e3e21db22034ecc032dc623e1a50d`);
  ...
  return (
    <main>
      <button onClick={handleBack} className="bg-blue-500 text-white p-2 rounded mb-5">
        Back
      </button>

      <section className='flex justify-around flex-wrap py-5'>
        <div className='max-w-sm'>
        <img className="rounded-t-lg" src={image} alt="movie_img" />
        </div>
